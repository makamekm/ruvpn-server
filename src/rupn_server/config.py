...
class ServerConfig:
    ...
    enable_bad_log_restart_watchdog: bool
    ...
    def key_hex(self) -> str:
        """Backward-compatible name used by the first env-pinning implementation."""
    ...
    def load() -> "ServerConfig":
        ...
        connection_type = ConnectionTypeRegistry.resolve(
            _env("RUPN_CONNECTION_TYPE", ConnectionTypeRegistry.default().name)
        )
        vp8_defaults = Vp8ChannelOptions.defaults()
        telemost_room = _env("RUPN_TELEMOST_ROOM", "") or _env("RUPN_TELEMOST_ROOM_ID", "")
        ...
        return ServerConfig(
            ...
            dns=_env("RUPN_DNS", ""),
            client_id=_device_name(force_random=rotate_on_start),
            client_key=_client_key(force_random=rotate_on_start),
            ...
            print_raw_uri=_env_bool("RUPN_PRINT_RAW_URI", False),
    ...
    def validate(self) -> None:
        ...
        if not re.fullmatch(r"[^\x00-\x1f\x7f?#%@$<>]{1,128}", self.client_id):
            raise ValueError("RUPN_DEVICE_NAME must be 1..128 characters without URI delimiters or controls")
        if not re.fullmatch(r"[0-9a-f]{64}", self.client_key):
            raise ValueError("RUPN_CLIENT_KEY/RUPN_KEY_HEX must be exactly 64 hexadecimal characters")
...
def _device_name(*, force_random: bool = False) -> str:
    configured = _env("RUPN_DEVICE_NAME", "") or _env("RUPN_CLIENT_ID", "")
    return configured if configured and not force_random else f"device-{secrets.token_hex(8)}"


def _client_key(*, force_random: bool = False) -> str:
    configured = (_env("RUPN_CLIENT_KEY", "") or _env("RUPN_KEY_HEX", "")).lower()
    return configured if configured and not force_random else secrets.token_hex(32)
